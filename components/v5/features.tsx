...
import { v4LovedPieces, v4QuoteOfDay, v4TrendingWriters } from "@/lib/v4-feed-data"
import { Avatar, HeartIcon, Tag } from "@/components/v5/primitives"
...
export function TrendingWriters() {
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--v5-border)] bg-[var(--v5-bg)]">
      {v4TrendingWriters.map((w, i) => (
        <div
          ...
          className={`flex items-center gap-2.5 px-3.5 py-3 min-[480px]:gap-3.5 min-[480px]:px-4 min-[480px]:py-3.5 ${
            i < v4TrendingWriters.length - 1 ? "border-b border-[var(--v5-border-soft)]" : ""
          }`}
        >
          ...
          <div className="shrink-0 text-right max-[479px]:mr-1.5 min-[480px]:mr-3">
            ...
            <div className="text-[9px] uppercase tracking-[0.08em] text-[var(--v5-subtle)]">followers</div>
          </div>
          <button
            type="button"
...
export function QuoteCallout() {
  return (
    <blockquote className="my-1 border-l-[3px] border-[var(--v5-fg)] py-0 pl-[18px]">
      <p className="mb-2.5 font-serif text-[15px] font-semibold italic leading-snug text-[var(--v5-fg)] min-[480px]:text-[17px] min-[480px]:leading-[1.55]">
