...
import type { ContentTag, V4PiecePost } from "@/lib/v4-feed-data"
...
const moreByAuthor: V4PiecePost[] = [
  {
    kind: "piece",
    type: "essay",
    date: "Apr 28",
...
export function ReadingRoom({ basePath = "/v7" }: { basePath?: string }) {
    ...
    return (text: string) => {
      ...
      return parts.map((part, i) => {
        ...
        return (
          <mark
            key={i}
            className="rounded-sm bg-[var(--v5-accent-soft)] px-0.5 text-[var(--v5-fg)]"
          >
            {part.text}
          </mark>
        )
  ...
  return (
    <div className="min-h-screen pb-32">
      ...
      >
        <div
          className="h-full bg-[var(--v5-accent)] transition-[width] duration-150"
          style={{ width: `${progress}%` }}
        />
      </div>

      <header className="sticky top-0 z-20 border-b border-[var(--v5-border)] bg-[color-mix(in_srgb,var(--v5-bg)_92%,transparent)] backdrop-blur-md">
        <div className="mx-auto flex h-12 max-w-6xl items-center justify-between px-4 min-[480px]:h-[52px] min-[480px]:px-6 lg:h-14 lg:px-8">
          <Link
            href={basePath || "/v7"}
            className="group flex items-center gap-2 text-[var(--v5-fg)] hover:text-[var(--v5-accent)]"
          >
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className={`flex h-8 w-8 cursor-pointer items-center justify-center rounded-full transition-colors min-[480px]:h-9 min-[480px]:w-9 ${
                saved
                  ? "text-[var(--v5-accent)]"
                  : "text-[var(--v5-muted)] hover:text-[var(--v5-fg)]"
              }`}
            ...
            <button
              type="button"
              className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full text-[var(--v5-muted)] transition-colors hover:text-[var(--v5-fg)] min-[480px]:h-9 min-[480px]:w-9"
              aria-label="Share"
            >
      ...
      >
        ...
        >
          ...
          <span className="font-medium text-[var(--v5-fg)]">{piece.author}</span>
      ...
      <section className="mx-auto mb-12 mt-16 max-w-[640px] px-5 min-[480px]:px-6 lg:max-w-[680px]">
        <div className="rounded-2xl border border-[var(--v5-border)] bg-[var(--v5-bg)] p-5 min-[480px]:p-7">
          <div className="flex items-start gap-4">
            <Avatar seed={piece.author} size={64} />
            <div className="min-w-0 flex-1">
              <p className="font-serif text-lg font-semibold leading-tight text-[var(--v5-fg)]">
                {piece.author}
              </p>
              ...
              <p className="mt-3 font-serif text-[14px] leading-relaxed text-[var(--v5-muted)] min-[480px]:text-[15px]">
                {piece.authorBio}
              </p>
            </div>
          </div>
          <div className="mt-5 flex flex-wrap gap-2">
            <button
              type="button"
              className="cursor-pointer rounded-full bg-[var(--v5-fg)] px-4 py-2 text-xs font-semibold tracking-wide text-[var(--v5-bg)] transition-opacity hover:opacity-90"
            >
              Follow
            </button>
            <button
              type="button"
              className="cursor-pointer rounded-full border border-[var(--v5-border)] bg-transparent px-4 py-2 text-xs font-semibold tracking-wide text-[var(--v5-fg)] transition-colors hover:border-[var(--v5-fg)]"
            >
              View profile
            </button>
          </div>
      ...
      <section className="mx-auto max-w-[640px] px-5 min-[480px]:px-6 lg:max-w-[680px]">
        ...
        {moreByAuthor.map((p, i) => (
          <PieceCard key={i} post={p} readHref={`${basePath}/read`} />
        ))}
      </section>

      <div className="pointer-events-none fixed bottom-4 left-0 right-0 z-30 flex justify-center px-4 min-[480px]:bottom-6">
        <div className="pointer-events-auto flex items-center gap-1 rounded-full border border-[var(--v5-border)] bg-[color-mix(in_srgb,var(--v5-bg)_94%,transparent)] p-1 shadow-[0_8px_24px_rgba(0,0,0,0.08)] backdrop-blur-md">
          <button
            type="button"
            ...
            className={`flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-2 text-xs font-medium tabular-nums transition-colors ${
              liked
                ? "bg-[var(--v5-accent-soft)] text-[var(--v5-accent)]"
                : "text-[var(--v5-fg)] hover:bg-[var(--v5-accent-soft)]"
            }`}
            aria-label="Like"
            aria-pressed={liked}
          >
          ...
          <span className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium tabular-nums text-[var(--v5-muted)]">
            <MessageCircle className="h-4 w-4" strokeWidth={1.5} />
            {piece.comments}
          </span>

          <span className="mx-1 h-5 w-px bg-[var(--v5-border)]" aria-hidden />
          ...
          <button
            type="button"
            className="flex cursor-pointer items-center justify-center rounded-full p-2 text-[var(--v5-muted)] transition-colors hover:text-[var(--v5-fg)]"
            aria-label="Share"
          >
          ...
          {highlights.length > 0 && (
            <>
              <span className="mx-1 h-5 w-px bg-[var(--v5-border)]" aria-hidden />
              <span className="px-2.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-[var(--v5-accent)]">
      ...
      {popover && (
        ...
        >
          <button
            ...
            className="pointer-events-auto cursor-pointer rounded-full bg-[var(--v5-fg)] px-3 py-1.5 text-[11px] font-semibold text-[var(--v5-bg)] shadow-[0_8px_24px_rgba(0,0,0,0.18)] hover:opacity-90"
          >
            Highlight
          </button>
        </div>
