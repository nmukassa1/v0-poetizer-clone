...
export function Divider({
  ...
  return (
    <div
      className={`my-7 flex items-center gap-2.5 max-[479px]:my-6 sm:my-7 lg:my-6 ${className}`}
    >
      <div className="h-px flex-1 bg-[var(--ink-border)]" />
      <span
        className={`font-serif text-[9px] font-semibold uppercase tracking-[0.12em] ${
          accent ? "text-[var(--ink-prompt-meta)]" : "text-[var(--ink-subtle)]"
        }`}
      >
        {label}
      </span>
...
export function Tag({ label }: { label: ContentTag | string }) {
  const c = tagStyles[label as ContentTag] ?? {
    bg: "#f1efe8",
    text: "#5f5e5a",
  };
  return (
    <span
      className="whitespace-nowrap rounded-full px-[7px] py-0.5 text-[9px] font-semibold uppercase tracking-[0.06em] flex items-center justify-center"
...
export function Actions({
  ...
  return (
    <div
      className={`mt-2.5 flex flex-wrap gap-4 text-[var(--ink-subtle)] ${textSize}`}
    >
      <button
        type="button"
