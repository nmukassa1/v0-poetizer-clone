...
function StreakCard({
  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border border-[var(--ink-border)] bg-[var(--ink-bg)] p-3.5 max-[479px]:flex-row min-[480px]:block min-[480px]:p-[18px]">
      ...
      <div className="hidden min-[480px]:mb-3.5 min-[480px]:flex min-[480px]:items-baseline min-[480px]:gap-1">
        ...
        <span className="text-[11px] text-[var(--ink-subtle)]">days</span>
      </div>
      ...
      <div className="flex flex-1 flex-row min-[480px]:block">
        <div className="mr-3 flex shrink-0 items-baseline gap-1 max-[479px]:flex min-[480px]:hidden">
          ...
          <span className="text-[10px] text-[var(--ink-subtle)]">days</span>
        </div>
        <div className="flex flex-1 gap-1">
          {DAYS.map((d, i) => (
            <div key={`${label}-${i}`} className="flex-1 text-center">
              <div
                className={`mb-1.5 h-[3px] rounded-sm ${active[i] ? "bg-[var(--ink-fg)]" : "bg-[var(--ink-border-soft)]"}`}
              />
              <span
                className={`font-sans text-[8px] ${active[i] ? "text-[var(--ink-fg)]" : "text-[#c8c4bb]"}`}
              >
                {d}
              </span>
            </div>
