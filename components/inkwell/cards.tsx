import type { Featured, SocialPost, WeeklyPrompt } from "@/lib/feed-data"
import Link from "next/link"
import { PieceCard } from "@/components/inkwell/piece-card"
import { Actions, Avatar, CommentIcon, HeartIcon, Tag } from "@/components/inkwell/primitives"
    ...
    <div>
      ...
      <div className="ink-scrollbar-hide flex gap-2.5 overflow-x-auto pb-1 [-webkit-overflow-scrolling:touch] lg:grid lg:grid-cols-2 lg:overflow-visible lg:pb-0 xl:grid-cols-4">
        {prompt.submissions.map((s, i) => (
          <div
            key={i}
            className="min-w-[150px] shrink-0 cursor-pointer rounded-[10px] border border-[var(--ink-prompt-border)] bg-white p-3 min-[480px]:min-w-[170px] min-[480px]:px-3.5 lg:min-w-0"
          >
            <Tag label={s.type} />
...
export function SocialPost({ post }: { post: SocialPost }) {
  return (
